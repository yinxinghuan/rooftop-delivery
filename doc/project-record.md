...
- 创建日期：2026-07-10
- 当前状态：已发布、已注册并已在平台列表可见；六关主题、动物主动干扰、真实落地物理与大胆邮政字阶均已上线
...
- 删除包裹组内会随箱体翻滚的 CircleGeometry 假阴影，只保留 PCFSoftShadowMap 实时软阴影，解决双重阴影和圆形贴图乱晃。
- 动物新增腿部步态、尾巴摆动、鸡翅扑动、身体起伏和跳扑反应；干扰从小型三维球改为屋顶上方圆柱区域，半径与冲量显著提高，实际命中会把包裹拍离原弹道。
- 落地物理升级为速度相关撞击、多段衰减弹跳、滑动 / 翻滚摩擦与出界再坠落，静止后才结算最终落点。
...
- 圆形假阴影相关几何体与逐帧更新 grep 均为零；待机 / 飞行截图只剩真实屋顶投影。猫和鸡连续动作帧验证巡逻位置、步态与扑翼发生变化；动物圆柱触发、强化冲量、屋顶撞击和接地摩擦均有纯函数边界测试。
...
9. 物理修复版本提交 `b80410e`，GitHub Pages 部署运行 `29093822693` 成功；线上 HTML 已切换至 `index-B7KsugIC.js`。该版本删除圆形假阴影，加入动物步态 / 跳扑、圆柱干扰判定、强化冲量，以及撞击—弹跳—滑动—滚动—出界再坠落的落地流程。
